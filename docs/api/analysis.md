...
> 注：源码中 Analysis 的子类包括 CutAnalysis（剖切）、ViewshedAnalysis（可视域）、SkylineAnalysis（天际线）、FloodAnalysis（水淹）、InSightAnalysis（通视）、CrossCutAnalysis（剖面）、HeightLimitAnalysis（限高，继承自 FloodAnalysis）。ExcavateAnalysis（挖方）不继承 Analysis，而是继承 `@maptalks/vt` 的 ExtrudePolygonLayer，不属于本页范围。
