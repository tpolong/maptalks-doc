...
该图层基于 [geojson-vt](https://github.com/mapbox/geojson-vt) 开源库，在worker中对GeoJSON数据实时切片后返回给主线程渲染绘制。
...
它是 [VectorTileLayer](/api/vector-tile-layer)的子类，同样继承了VectorTileLayer上的方法和配置。
...
|features         | Boolean         | 瓦片是否返回feature数据，对GeoJSON图层只返回要素id | true |
|tileBuffer       | Number          | 瓦片缓冲大小             | 64 |
|extent           | Number          | 瓦片坐标范围（extent，即几何编码精度） | 8192 |
|simplifyTolerance| Number          | 瓦片简化容差（越大越简化）  | 3 |
|generateOMBB     | Boolean         | 是否生成最小外接矩形（OMBB，加速拾取） | true |
|convertFn        | String          | 数据转换函数字符串（在worker中执行convert(data)） | null |
