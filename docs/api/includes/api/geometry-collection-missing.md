...
获取几何体在当前 zoom 下的二维 extent，无 map 时返回 null
...
获取几何图形外壳的 ring 坐标，可选成员，仅面状子类实现
...
获取 geometry 的 coordinate 数组，可选成员，仅点线面子类实现
...
设置 geometry 的 coordinates 并触发 shapechange
...
geometry 从 layer 移除时的可选生命周期回调，集合类未实现
...
geometry 添加到 layer 时的可选生命周期回调，集合类未实现
