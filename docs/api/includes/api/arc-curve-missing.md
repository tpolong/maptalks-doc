...
设置曲线的新 coordinates，并触发 shapechange
...
判断该 geometry 是否带有洞，ArcCurve 等线状几何体返回假值
...
获取 symbol 文本按 properties 解析后的文本描述
...
获取 geometry 在当前 zoom 下投影到屏幕像素的范围，无地图时返回 null
...
geometry 隐藏时的生命周期回调，关闭其菜单与信息窗口
...
形状变化时的生命周期回调，清缓存重绘并派发 shapechange 事件
...
位置变化时的生命周期回调，清缓存重绘并派发 positionchange 事件
...
symbol 变化时的回调，刷新 symbol 并派发 symbolchange 事件
...
配置项变更时的生命周期回调，写入新 properties 或重绘
...
获取 geometry 的高度，优先取 properties 再取坐标 z 值，没有则为 0
...
设置该 geometry 的 altitude，同步改写 coordinate 的 z 值
...
获取该 geometry 内环即洞的坐标数组，没有洞时为空数组
...
geometry 从 layer 移除时的可选生命周期回调，ArcCurve 未实现
...
geometry 添加到 layer 时的可选生命周期回调，ArcCurve 未实现
