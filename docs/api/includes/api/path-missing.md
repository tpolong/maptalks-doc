...
获取当前 zoom 下 geometry 的 2D 像素 extent
...
获取 geometry 的 altitude，返回数值或高度数组
...
设置 path 的 altitude，并同步改写 coordinate 的 z 值
...
获取 path 所有 coordinate 中最小的 altitude
...
获取 geometry 所有 coordinate 中最大的 altitude
...
获取 path 外环的 coordinate 数组，仅 Polygon 等子类实现
...
获取 geometry 集合内的子 geometry 数组，仅集合类子类实现
...
获取 geometry 的 coordinate，按类型返回坐标或嵌套数组
...
设置 path 的 coordinate 并更新其几何状态，由子类实现
