...
获取 geometry 范围对应的 outline polygon，无则为空
...
设置 curve 的新坐标并触发 shapechange 事件
...
获取曲线的坐标数组，即构成曲线的 coordinate 列表
...
* options（可选） `(AnimationOptionsType | animateShowCallback) = {}` =null] animation options
* cb（可选） `animateShowCallback` =null] callback function in animation, function parameters: frame, currentCoord
...
获取由 symbol 的 textName 与 properties 生成的文本描述
...
获取当前 zoom 下 geometry 的屏幕像素范围，未加地图时返回 null
...
geometry 被隐藏时的回调，关闭其菜单与 info window
...
形状改变后的回调，清理缓存并重绘，再触发 shapechange 事件
...
geometry 位置变化时的回调，清缓存重绘并触发 positionchange
...
symbol 变化后的回调，刷新绘制并触发 symbolchange 事件
...
配置更新时的回调，应用 properties 并在箭头等参数变化时重绘
...
获取 geometry 的海拔，取 properties 或坐标 z 值，缺省为 0
...
设置 geometry 高度，同步更新 coordinates 的 z 值并重绘
...
获取几何图形外壳的 ring 坐标，可选成员，线状图形不实现
...
获取几何集合内的子 geometry 数组，可选成员，仅集合类实现
...
geometry 从 layer 移除时的可选生命周期回调，Curve 未实现
...
获取旋转时叠加的角度偏移，可选成员，仅 sector 等特殊图形实现
...
geometry 添加到 layer 时的可选生命周期回调，Curve 未实现
