...
获取几何图形 extent 生成的轮廓 Polygon，无 painter 时返回 null
...
获取几何图形的文本描述，由 symbol 与 textContent 分行计算并缓存
...
获取几何图形在视图点坐标系中的 2D extent，无 map 时返回 null
...
获取几何图形的高度，取自 altitude 属性或坐标 z 值，缺省为 0
...
可选接口方法，集合类几何图形用它返回子 geometry 数组
