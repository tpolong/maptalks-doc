...
获取曲线范围对应的 outline polygon，无 painter 时返回 null
...
设置二次贝塞尔曲线的 coordinate 数组，并按新坐标重新投影
...
获取曲线的 coordinates，即构成曲线的 coordinate 列表
...
获取由 symbol 的 textName 与 properties 生成的文本描述并缓存
...
获取当前 zoom 下曲线的 2D 像素 extent，不在 map 上返回 null
...
位置变化时的回调，清缓存重绘并触发 positionchange
...
symbol 变化后的回调，刷新 symbol 并触发 symbolchange
...
获取曲线海拔，取 properties 或 coordinate 的 z 值，缺省为 0
...
设置曲线的 altitude，同步改写 coordinate 的 z 值并重绘
...
获取曲线所有 coordinate 中最小的 altitude
...
获取曲线所有 coordinate 中最大的 altitude
...
曲线从 layer 移除时的可选回调，QuadBezierCurve 未实现
...
曲线加入 layer 时的可选回调，QuadBezierCurve 未实现
