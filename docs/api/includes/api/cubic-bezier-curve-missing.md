...
获取 geometry 范围对应的 outline polygon，无则返回 null
...
获取缓存的文本描述，由 symbol 的 textName 与 properties 生成
...
获取当前 zoom 下 geometry 的屏幕像素范围，未加到地图时返回 null
...
geometry 位置变化时的回调，清缓存重绘并触发 positionchange 事件
...
配置更新时的回调，应用 properties 并在箭头或平滑参数变化时重绘
...
设置 geometry 的 altitude，同时改写坐标的 z 值
...
从 layer 移除时的可选回调，CubicBezierCurve 未实现
...
获取图形旋转时叠加的角度偏移量，供 sector 等特殊图形使用
