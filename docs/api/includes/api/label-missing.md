...
导出 JSON，含 feature 与 content，结果中不含 symbol 字段
...
按 symbol 键名的 text 前缀拆成文本与盒子两部分分别设置
...
TextMarker 声明的可选方法，设置 label 文本样式，由 TextBox 实现
...
TextMarker 声明的可选方法，设置 label 盒子 symbol，由 TextBox 实现
...
生成与该 marker 包围盒同尺寸的方形边框 marker
...
获取 geometry 在当前 zoom 下的像素 extent，无 map 时返回 null
...
geometry 隐藏时回调，关闭其菜单与 info window
...
symbol 变化回调，刷新 symbol 并触发 symbolchange
...
配置变更回调，应用 properties 并在箭头或平滑选项变化时重绘
...
获取 geometry 的 altitude，取自 properties 的 altitude 或坐标 z，无则返回 0
...
设置 geometry 的 altitude，同步 properties 与坐标 z 后重绘
...
获取 geometry 上所有顶点 altitude 的最小值
...
获取 geometry 上所有顶点 altitude 的最大值
