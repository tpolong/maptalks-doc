...
symbol 变化时的生命周期回调，刷新 symbol 并派发 symbolchange
...
配置项变更时的生命周期回调，写入新 properties 并在需要时重绘
...
获取 geometry 的高度，优先取 properties 再取坐标 z 值
...
设置 geometry 的高度，同步更新 properties 与坐标的 z 值
...
geometry 被移除时的生命周期回调，解绑连接源与目标上的事件
...
geometry 添加到 layer 时的生命周期回调，注册连接事件并刷新连线坐标
