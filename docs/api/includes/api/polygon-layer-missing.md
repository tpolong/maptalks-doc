...
options 变更后的回调，把 bloom 开关同步给 renderer
...
获取多边形绘制所需的 polygonOffset 层数，有高度时为 0，无高度时为 2
...
设置图层 polygonOffset 及总数，供 GroupGLLayer 统一管理
...
geometry 加入图层时按图层 style 设置其 symbol
...
响应几何体事件，按事件类型同步 renderer 与图层缓存
