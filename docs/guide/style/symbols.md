...
本文档包含了矢量的样式属性，三维的样式由渲染材质定义，具体请参考[材质说明文档](/guide/style/material)。
...
> [!NOTE]
> 属性名上带*号的，表示该属性不支持[function-type](/guide/style/function-type)。
...
**String**，必须有合法的markerType。矢量图标边框的模式填充图片地址，支持url或者base64字符串。
...
| --- | --- | --- |
| ssr | Boolean | 屏幕空间反射 |
| texWaveNormal | String | 波浪法线贴图 |
| texWavePerturbation | String | 波浪扰动贴图 |
| waterBaseColor | Number[] | 水面基础色 |
| contrast | Number | 对比度 |
| hsv | Number[] | HSV 调整 |
| uvScale | Number | UV 缩放 |
| animation | Boolean | 波浪动画 |
...
| bloom | Boolean | 是否辉光 |
| ssr | Boolean | 屏幕空间反射 |
| polygonOpacity | Number | 多边形不透明度 |
| material | Object | PBR 材质（LitMaterial），字段见[材质说明文档](/guide/style/material) |

### PolygonLayer / ExtrudePolygonLayer 补充

- `uvOffsetInMeter`（Boolean，默认 false）：uv 偏移是否按米计。
- `polygonPatternFileWidth` / `polygonPatternFileHeight` / `polygonPatternFileOrigin`：填充纹理的宽、高与原点设置。
