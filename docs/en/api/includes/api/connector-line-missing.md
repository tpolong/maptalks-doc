...
Gets an outline Polygon of the geometry extent.
...
Sets new coordinates to the connector line and fires shapechange.
...
Gets the coordinates of the connector line, the two points between source and target.
...
Whether the geometry has any holes inside.
...
Gets the text descriptions resolved from the symbol text content and properties.
...
Gets the geometry projected extent in screen pixels at the current zoom, null without a map.
...
Whether the geometry has been rotated.
...
Lifecycle callback when the geometry is hidden; closes its menu and info window.
...
Lifecycle callback after a shape change; clears caches, repaints and fires shapechange.
...
Lifecycle callback after a position change; clears caches, repaints and fires positionchange.
...
Lifecycle callback after a symbol change; refreshes the symbol and fires symbolchange.
...
Lifecycle callback on option changes; applies new properties and repaints when needed.
...
Gets the geometry altitude from properties or coordinate z values.
...
Whether the geometry has any non-zero altitude value.
...
Sets the geometry altitude, updating properties and coordinate z values.
...
Gets the minimum altitude of the geometry.
...
Gets the maximum altitude of the geometry.
...
Gets the hole rings coordinates of the geometry, an empty array if it has none.
...
Gets the outer ring coordinates of the geometry.
...
Gets the child geometries contained in this geometry.
...
Lifecycle callback when the geometry is removed; unbinds events on its connect source and target.
...
Gets the offset angle used when rotating the geometry.
...
Lifecycle callback when the geometry is added to a layer; registers connect events and updates coordinates.
