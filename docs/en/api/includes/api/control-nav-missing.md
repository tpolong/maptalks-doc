...
Optional callback after the control is added to a map, not overridden by Nav.
...
Optional callback after the control is removed from the map, not overridden by Nav.
...
Recompute and update the control position when the config contains position.
