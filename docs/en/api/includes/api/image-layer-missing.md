...
Hook called when the layer finishes loading, empty by default and overridable
...
Callback after options change, triggering redraw and attribution update
...
Callback fired after the layer renderer is created, subclasses may override
...
Callback fired after the layer canvas is created, subclasses may override it
...
Callback fired when the layer is removed from its map, subclasses may override
...
Export the layer as JSON containing its type, id and options
...
Get the child layer array, implemented only by group layer types
