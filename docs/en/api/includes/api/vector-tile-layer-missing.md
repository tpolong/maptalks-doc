...
Get the main tiles and parent tile grids needed to render at the given zoom
...
Check whether a tile is within the parent tile zoom range to be kept
...
Export the layer profile json, which can rebuild the instance with fromJSON
...
Get the layer's min zoom to display
...
Get the layer's max zoom to display
...
Generate a unique tile id from the tile x, y and zoom
...
Hook called after the layer finishes loading, overridden by subclasses, empty by default
...
Callback after the layer renderer is created, may be overridden by subclasses
...
Callback after the layer canvas is created, may be overridden by subclasses
