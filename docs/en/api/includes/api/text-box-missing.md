...
Lifecycle callback when added to a layer, refreshing the textbox symbol
...
Export the textbox JSON description without the symbol field
...
Set the symbol, splitting text prefixed props from box props
...
Set the textbox background box style, implemented by subclasses
...
Get the box style of the textbox, implemented by subclasses
...
Get a square outline marker sized by the textbox's containerExtent
...
Get the text description computed from the text content and size symbol, cached
...
Get the textbox's 2D pixel extent at current zoom, or null when it is not on a map
...
Whether the geometry has a rotation angle and pivot set
...
Callback clearing cache, repainting and firing shapechange on shape change
...
Callback refreshing symbol, rebuilding size cache and firing symbolchange
...
Callback after config updates options, syncing properties and repainting as needed
...
Get the textbox altitude, from its altitude property or coordinate z, defaulting to 0
...
Set the geometry altitude and update the z value of its coordinates
...
Get the minimum altitude among the textbox's coordinates
...
Get the maximum altitude among the textbox's coordinates
...
Get the geometry's hole coordinates, implemented only by areal geometry subclasses
...
Get the geometry's outer shell coordinates, implemented only by areal geometry subclasses
...
Get the child geometries of a geometry collection, implemented only by collection subclasses
...
Get the fixed angle offset used when rotating around the center, implemented only by Sector
