...
Get an outline Polygon built from the geometry extent, or null when it has no painter.
...
Whether the geometry has any holes inside; a circle never has any.
...
Get the geometry's cached text description, computed by splitting textContent into rows.
...
Get the geometry's 2D extent in view point coordinates at GL resolution, null without a map.
...
Get the geometry's altitude from its altitude property or coordinate z, defaulting to 0.
...
Get the minimum altitude of the geometry, computed lazily and then cached.
...
Get the maximum altitude of the geometry, computed lazily and then cached.
...
Optional interface method returning the child geometries of a collection geometry.
...
Optional lifecycle callback invoked when the geometry is removed from a layer.
...
Optional interface method returning the offset angle used when rotating the geometry coordinates.
...
Optional lifecycle callback invoked when the geometry is added to a layer.
