...
Returns an outline polygon of the curve extent, null without a painter
...
Sets the coordinate array of the quadratic bezier curve and reprojects it
...
Returns the curve coordinates, the coordinate list forming the curve
...
Whether the geometry has holes; optional member not implemented by curves
...
Returns the curve 2D pixel extent at current zoom, null when not on a map
...
Whether the curve has been rotated with both an angle and a pivot
...
Callback when the curve is hidden, closing its menu and info window
...
Callback after shape change, clearing caches, repainting and firing shapechange
...
Callback on position change, clearing caches, repainting and firing positionchange
...
Callback after symbol change, refreshing the symbol and firing symbolchange
...
Config callback applying properties and repainting on arrow or smooth changes
...
Returns the curve altitude from properties or coordinate z, 0 when absent
...
Whether the curve has a non-zero altitude
...
Sets the curve altitude, syncing coordinate z values and repainting
...
Returns the minimum altitude among the curve coordinates
...
Returns the maximum altitude among the curve coordinates
...
Returns hole rings of the geometry; optional member not implemented by curves
...
Returns shell ring coordinates; optional member not implemented by curves
...
Returns child geometries of the collection; optional member not implemented by curves
...
Optional lifecycle callback when removed from a layer; not implemented by QuadBezierCurve
...
Returns the rotation offset angle; optional member not implemented by curves
...
Optional lifecycle callback when added to a layer; not implemented by QuadBezierCurve
