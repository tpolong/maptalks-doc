...
Get the coordinates of the child geometries in the collection.
...
Set new coordinates by rebuilding every child geometry of the collection
...
Get the union container extent of all geometries in the collection.
...
Get all the geometries contained in the geometry collection.
...
Apply the config changes to every geometry inside the collection
...
Get the symbol, or a children array of child geometry symbols when none is set
...
Set the symbol, a children object is assigned in order to child geometries, otherwise shared
...
Start editing all geometries in the collection, with an optional editing symbol
...
End editing of the collection and restore the symbol and draggable option.
...
Undo one edit step of the child geometries and fire undoedit
...
Redo one edit step of the child geometries and fire redoedit
...
Whether all undoable edits of the child geometries have been undone
...
Whether all redoable edits of the child geometries have been redone
...
Get the text description computed from text content and sizeSymbol, cached
...
Whether the geometry has both a rotate angle and a pivot set
...
Clear cache, repaint and fire shapechange when the shape changes
...
Clear cache, repaint and fire positionchange when the position changes
...
Refresh symbol, rebuild sizeSymbol and fire the event when symbol changes
...
Get the geometry altitude, falling back to coordinate z values or 0.
...
Whether the geometry has a non zero altitude set
...
Set the altitude of the geometry and update the z values of its coordinates
...
Get the hole coordinate array of a polygon geometry, implemented by few subclasses.
...
Get the coordinate ring of the geometry shell, only some subclasses implement it
...
Get the rotation angle offset when rotating around the center, only Sector implements it
...
Callback invoked after the geometry is added to a layer, called by the framework
