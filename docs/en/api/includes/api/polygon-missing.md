...
Callback when the geometry is hidden, closing its menu and infoWindow
...
Clear caches, repaint and fire shapechange when the shape changes
...
Clear caches, repaint and fire positionchange when the position changes
...
Refresh the painter symbol and rebuild sizeSymbol when the symbol changes
...
Callback after config updates options, syncing properties and repainting
...
Get the geometry altitude as a number or an array of altitudes
...
Get child geometries of a collection, implemented by collection subclasses only
...
Get the rotation angle offset about the center, implemented only by Sector
...
Callback invoked after the geometry is added onto a layer
