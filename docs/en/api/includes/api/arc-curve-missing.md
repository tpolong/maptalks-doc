...
Set new coordinates for the curve and fire shapechange.
...
Gets the coordinates array of the curve geometry.
...
Whether the geometry has holes; line-based geometries such as ArcCurve return falsy.
...
Whether the geometry has been rotated with an angle and a pivot.
...
Lifecycle callback on option changes; applies new properties or repaints.
...
Gets the geometry altitude from properties or coordinate z values, 0 when there is none.
...
Optional lifecycle callback when the geometry is removed from a layer; not implemented by ArcCurve.
...
Optional lifecycle callback when the geometry is added to a layer; not implemented by ArcCurve.
