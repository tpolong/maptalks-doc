...
Get all geometries of the collection as an array
...
Apply config changes to every geometry in the collection
...
Get the symbol, or a children array of child geometries when it has none
...
End editing, restore the pre-edit symbol and visibility, and fire editend
...
Whether all child geometries edits have already been undone
...
Whether all child geometries edits have already been redone
...
Get the 2D pixel extent of the collection at the current zoom
...
Whether the geometry has a rotation angle and a pivot set
...
Refresh symbol, rebuild sizeSymbol and fire symbolchange when symbol changes
...
Get the altitude of the geometry as a number or an altitude array
...
Set the geometry altitude and rewrite the z value of its coordinates
...
Get the minimum altitude among all the geometry coordinates
...
Get hole coordinate rings of a polygon-like geometry, implemented by subclasses
...
Get the shell coordinates of the geometry, implemented by some subclasses only
...
Callback when the geometry is removed from its layer, called by the framework
...
Get the angle offset when rotating around the center, only Sector implements it
...
Callback after the geometry is added to a layer, called by the framework
