...
Reuses the Polygon implementation to build an outline polygon from the extent.
...
Sets new coordinates to the connector line.
...
Returns the geometry's coordinates, as coordinates or nested arrays by type.
...
Returns the cached text description computed from text content and sizeSymbol.
...
Returns the geometry's altitude as a single value or an array of altitudes.
...
Returns the hole coordinate rings of a polygon-like geometry, only some subclasses implement it.
...
Returns the shell coordinate ring of the geometry, only some subclasses implement it.
...
Returns the rotate offset angle in degrees, implemented only by Sector now.
