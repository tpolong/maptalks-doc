...
Gets an outline polygon built from the geometry's extent.
...
Gets the text layout description computed from the geometry's symbol.
...
Gets the geometry's 2D extent at the map's GL resolution, null when it has no map.
...
Lifecycle callback when the geometry is hidden, closing its menu and info window.
...
Lifecycle callback after the shape changes, clearing caches, repainting and firing shapechange.
...
Lifecycle callback after the symbol changes, refreshing the painter and firing symbolchange.
...
Lifecycle callback after config changes, reapplying properties and repainting.
...
Gets the geometry's altitude from its altitude property or coordinate z, or 0 if unset.
...
Whether the geometry has any altitude set.
...
Sets the geometry's altitude on its properties and coordinate z.
...
Gets the geometries contained in the geometry collection.
...
Gets the offset angle used to compensate the geometry's rotation.
