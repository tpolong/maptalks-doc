...
Get coordinates of all child geometries in the collection
...
Set new coordinates to the collection, rebuilding its child geometries.
...
Get the union of all child geometry container extents
...
Get the child geometries of the collection
...
Callback on config change, syncing properties and repainting when needed.
...
Get symbol, or a children array of child symbols when none is set
...
Set the symbol, applying a children array to child geometries in order.
...
Start editing every geometry in the collection.
...
End editing, restore symbol and visibility and fire editend
...
Undo one step on the next undoable geometry of the collection.
...
Redo one step on the next redoable geometry of the collection.
...
Whether every geometry in the collection has no more steps to undo.
...
Whether every geometry in the collection has no more steps to redo.
...
Get the cached text description computed from text content and sizeSymbol
...
Get the geometry 2D pixel extent at current zoom
...
Whether the geometry has both a rotate angle and a rotate pivot set.
...
Callback on shape change, clearing cache, repainting, firing shapechange.
...
Callback on position change, clearing cache, repainting, firing positionchange.
...
Callback on symbol change, refreshing the painter and firing symbolchange.
...
Get the geometry altitude, a number or an array of altitudes
...
Set the geometry altitude and also rewrite the z value of its coordinates.
...
Get the minimum altitude among the geometry coordinates
...
Get hole rings of a polygon-like geometry, line strings do not implement it
...
Get the geometry shell coordinates, line strings do not implement it
...
Lifecycle callback invoked when the geometry is removed from its layer.
...
Get the angular offset when rotating about center, only Sector implements it
