...
Get a square outline marker whose size is taken from the container extent
...
Get the text description computed from text content and sizeSymbol and cached
...
Get the geometry 2D pixel extent at the current zoom
...
Whether the geometry has both a rotation angle and a rotation pivot set
...
Clear cache, repaint and fire the shapechange event when the shape changes
...
Refresh symbol, rebuild sizeSymbol and fire the symbolchange event on change
...
Get the hole coordinate arrays of a polygon, implemented by some subclasses
...
Get the outer ring coordinate array of the geometry, not implemented by marker classes
...
Get child geometries of a collection, implemented by collection subclasses
